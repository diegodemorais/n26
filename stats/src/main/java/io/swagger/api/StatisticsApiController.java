...
public class StatisticsApiController implements StatisticsApi {
    ...
    public ResponseEntity<Statistic> getStatistics() {
        ...
        if (accept != null && accept.contains("application/json")) {
            try {
                return new ResponseEntity<Statistic>(objectMapper.readValue("{  \"amount\" : 0.8008281904610115,  \"avg\" : 6.027456183070403,  \"min\" : 5.962133916683182,  \"max\" : 1.4658129805029452,  \"count\" : 5}", Statistic.class), HttpStatus.NOT_IMPLEMENTED);
            } catch (IOException e) {
                log.error("Couldn't serialize response for content type application/json", e);
                return new ResponseEntity<Statistic>(HttpStatus.INTERNAL_SERVER_ERROR);
            }
        }
